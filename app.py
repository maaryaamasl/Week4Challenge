...
st.write("""This dashboard presents an interactive analysis of analyst earnings estimates (VALUE) 
and actual reported earnings (ACTUAL) using the IBES dataset. The main objective of this analysis 
is to explore how well analyst predictions align with real earnings outcomes and to examine whether 
forecast accuracy has improved over time.

The dashboard provides a variety of interactive visualizations designed to support data exploration, 
identify outliers, analyze relationships between variables, and test key hypotheses. Specifically, 
the analysis focuses on two main questions:
1. Is there a positive correlation between analyst estimates and actual earnings?
2. Has the accuracy of these estimates improved across the years?

These insights aim to support better understanding of forecast performance and assist 
in improving future prediction strategies.""")
...
st.header("Boxplot of ACTUAL Earnings by Company (Outliers Included)")
...
st.header("Insights")
st.write("""The analysis shows a positive correlation between analyst estimates (VALUE) and actual reported earnings (ACTUAL), 
supporting the first hypothesis. This suggests that analyst forecasts generally reflect real earnings outcomes, 
though with varying degrees of accuracy.

Exploration of the data using histograms and boxplots highlights the presence of outliers and 
differences in earnings across companies. The correlation heatmap identifies key relationships between variables, 
which may influence the accuracy of predictions.

Forecast error analysis over time reveals fluctuations in the accuracy of analyst estimates, 
with certain periods showing improvement while others continue to exhibit higher levels of error. 
These findings suggest that while forecasting methods may be improving, consistent accuracy remains a challenge.""")

st.header("✅ Recommendations")

st.write("""
To improve the accuracy and reliability of analyst earnings forecasts, it is recommended to regularly 
monitor forecast performance, especially focusing on periods or companies where prediction errors are high. 
Identifying these cases can help in refining forecasting models and improving outcomes.

Forecasting strategies should leverage the variables that demonstrate strong correlations with actual earnings, 
as revealed through the correlation heatmap and pairplot analysis. Incorporating these key factors can enhance 
the predictive power of estimation models.

It is also advisable to apply effective outlier detection and handling methods, particularly for companies 
that consistently show extreme values. Addressing these outliers can contribute to more stable and reliable forecasts.

This dashboard serves as a flexible tool for ongoing evaluation, supporting data-driven decisions 
and continuous improvement in earnings analysis and forecasting.
""")